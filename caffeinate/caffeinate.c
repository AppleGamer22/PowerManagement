...
int main(int argc, char *argv[]) {
	...
	if (argc - optind) {
		...
		if (waitforpid) {
			disp_src = dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, waitforpid, DISPATCH_PROC_EXIT, dispatch_get_main_queue());
...
int createAssertions(const char *progname, AssertionFlag flags, long timeout) {
	...
	if (progname) {
		(void)snprintf(assertionDetails, sizeof(assertionDetails), "caffeinate asserting on behalf of \'%s\' (pid %d)", progname, getppid());
	} else if (waitforpid) {
		if (timeout) {
			snprintf(assertionDetails, sizeof(assertionDetails), "caffeinate asserting on behalf of Process ID %d for %ld secs", waitforpid, timeout);
		} else {
			snprintf(assertionDetails, sizeof(assertionDetails), "caffeinate asserting on behalf of Process ID %d", waitforpid);
		}
	} else if (timeout) {
		(void)snprintf(assertionDetails, sizeof(assertionDetails), "caffeinate asserting for %ld secs", timeout);
	} else {
		(void)snprintf(assertionDetails, sizeof(assertionDetails), "caffeinate asserting forever");
	}
	...
	for (i = 0; i < sizeof(assertionMap) / sizeof(AssertionMapEntry); ++i) {
		...
		if ((entry->assertionFlag == kUserActiveAssertionFlag) && (timeout == 0)) {
			// Force a 5sec timeout on user active assertions
			timeout = 5;
		}

		result = IOPMAssertionCreateWithDescription(entry->assertionType, CFSTR(kAssertionNameString), assertionDetailsString, kHumanReadableReason, kLocalizationBundlePath, (CFTimeInterval)timeout, kIOPMAssertionTimeoutActionRelease, &assertionID);
